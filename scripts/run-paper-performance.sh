...
usage() {
  cat <<'EOF'
Usage:
  scripts/run-paper-performance.sh --prepared-bundle DIR [--output-root DIR]
  scripts/run-paper-performance.sh --simulate --output-root DIR --run-id ID [OPTIONS]

Formal execution:
  --prepared-bundle DIR  Sealed bundle produced by preparation
  --output-root DIR      Defaults to artifacts/paper-performance

Diagnostic simulator options:
  --simulate                  Use synthetic data; always marks the run diagnostic
  --run-id ID                 New artifact directory name
  --concurrencies LIST        Default: 1,2
  --repetitions COUNT         Default: 2
  --warmup-seconds SECONDS    Default: 1
  --measurement-seconds SEC   Must be 1
  --shuffle-seed SEED         Default: 42

The orchestrator never retries a failed cell. Do not run a formal matrix more
than once. Diagnostics are refused under artifacts/paper-performance.
EOF
}
...
while (($# > 0)); do
  case "$1" in
    --simulate)
      ...
      shift
      ;;
    --prepared-bundle|--output-root|--run-id|--concurrencies|--repetitions|--warmup-seconds|--measurement-seconds|--shuffle-seed)
...
if [[ $mode == simulate ]]; then
  [[ -z $prepared_bundle ]] || { printf '%s\n' '--prepared-bundle cannot be used with --simulate' >&2; exit 2; }
  [[ -n $output_root ]] || { printf '%s\n' '--output-root is required for simulation' >&2; exit 2; }
  [[ -n $run_id ]] || { printf '%s\n' '--run-id is required for simulation' >&2; exit 2; }
  case "$output_root" in
    */artifacts/paper-performance|*/artifacts/paper-performance/*)
      printf '%s\n' 'diagnostic simulation cannot use artifacts/paper-performance' >&2
      exit 2
      ;;
  esac
  export CARGO_TARGET_DIR=${CARGO_TARGET_DIR:-$root/target/paper-performance-cli}
  exec cargo run --quiet -p paper-benchmark --bin dtgproxy-paper-benchmark -- \
    simulate \
...
[[ -n $prepared_bundle ]] || { printf '%s\n' '--prepared-bundle is required for a real run' >&2; exit 2; }
[[ -z $run_id && $concurrencies == 1,2 && $repetitions == 2 && $warmup_seconds == 1 && $measurement_seconds == 1 && $shuffle_seed == 42 ]] || {
  printf '%s\n' 'diagnostic simulator options require --simulate' >&2
  exit 2
}
...
def reject(message):
    raise SystemExit(f"run-paper-performance: {message}")
...
executor = executable(executor_value, "executor_sha256", "executor")
orchestrator = executable(orchestrator_value, "orchestrator_sha256", "orchestrator")
print(bundle / spec_name)
...
output_root=${output_root:-$root/artifacts/paper-performance}
export DTGPROXY_PAPER_RUNTIME_MANIFEST=$runtime_manifest
exec "$orchestrator_bin" run \
  --spec "$spec" \
  --output-root "$output_root" \
  --executor "$executor"
