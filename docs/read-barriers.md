...
Phase 2 exposes two explicit read modes. Neither mode accepts a raw Adapter reference from the
caller; `ShardQueryExecutor` validates the typed Temporal IR, acquires a `ReadPermit`, and only then
constructs the local executor over the selected Replica.
...
For an `AS OF` plan, the required time is its transaction selector. For `DIFF`, it is the upper
transaction bound. A `CURRENT` plan is rejected before Adapter access. `ShardQueryExecutor` also
fences the plan partition against the target Shard before requesting ReadIndex.
...
versioned `ClosedTimestampTick` even on an otherwise idle Shard. Once committed and applied, Phase
2 advances `closed_ts`, `resolved_ts`, and `adapter_applied_ts` in the same atomic state-machine
batch, making `safe_ts` visible to Followers. An isolated Follower cannot use the new timestamp:
its applied index remains below the subsequent ReadIndex proof. Phase 3 replaces the temporary
`resolved_ts = closed_ts` shortcut with Intent-aware resolution.
...
contract is the service boundary for the durable multi-process runtime built in Phase 2 acceptance.
