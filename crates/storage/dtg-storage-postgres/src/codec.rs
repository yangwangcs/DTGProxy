...
pub(crate) fn encode_properties(properties: &Properties) -> Result<Vec<u8>, StorageError> {
    let mut encoder = Encoder::default();
    encoder.properties(properties)?;
    Ok(encoder.finish())
}

pub(crate) fn decode_properties(bytes: &[u8]) -> Result<Properties, StorageError> {
    let mut decoder = Decoder::new(bytes);
    let properties = decoder.properties(0)?;
    decoder.finish()?;
    Ok(properties)
}
...
impl Encoder {
    ...
    fn bytes(&mut self, value: &[u8]) -> Result<(), StorageError> {
        let length = u32::try_from(value.len())
            .map_err(|_| StorageError::Internal("PostgreSQL value is too large".into()))?;
        self.u32(length);
    ...
    fn properties(&mut self, properties: &Properties) -> Result<(), StorageError> {
        let length = u32::try_from(properties.len())
            .map_err(|_| StorageError::Internal("too many PostgreSQL properties".into()))?;
        self.u32(length);
    ...
    fn value(&mut self, value: &Value, depth: usize) -> Result<(), StorageError> {
        if depth > MAX_NESTING {
            return Err(StorageError::Internal(
                "PostgreSQL value nesting exceeds the provider bound".into(),
            ));
        }
        match value {
            ...
            Value::List(values) => {
                ...
                let length = u32::try_from(values.len())
                    .map_err(|_| StorageError::Internal("PostgreSQL list is too large".into()))?;
                self.u32(length);
                for value in values {
            ...
            Value::Map(values) => {
                self.u8(7);
                let length = u32::try_from(values.len())
                    .map_err(|_| StorageError::Internal("PostgreSQL map is too large".into()))?;
                self.u32(length);
...
impl<'a> Decoder<'a> {
    ...
    fn finish(&self) -> Result<(), StorageError> {
        if self.offset == self.bytes.len() {
            ...
            Err(corrupt("trailing bytes in PostgreSQL typed value"))
        }
    }
    ...
    fn take(&mut self, length: usize) -> Result<&'a [u8], StorageError> {
        let end = self
            ...
            .ok_or_else(|| corrupt("truncated PostgreSQL typed value"))?;
    ...
    fn u32(&mut self) -> Result<u32, StorageError> {
        Ok(u32::from_be_bytes(
            self.take(4)?
                .try_into()
                .map_err(|_| corrupt("invalid PostgreSQL u32"))?,
        ))
    }
    ...
    fn u64(&mut self) -> Result<u64, StorageError> {
        Ok(u64::from_be_bytes(
            self.take(8)?
                .try_into()
                .map_err(|_| corrupt("invalid PostgreSQL u64"))?,
        ))
    }
    ...
    fn u128(&mut self) -> Result<u128, StorageError> {
        Ok(u128::from_be_bytes(
            self.take(16)?
                .try_into()
                .map_err(|_| corrupt("invalid PostgreSQL u128"))?,
        ))
    }
    ...
    fn i64(&mut self) -> Result<i64, StorageError> {
        Ok(i64::from_be_bytes(
            self.take(8)?
                .try_into()
                .map_err(|_| corrupt("invalid PostgreSQL i64"))?,
        ))
    }

    fn bytes(&mut self) -> Result<Vec<u8>, StorageError> {
        let length =
            usize::try_from(self.u32()?).map_err(|_| corrupt("invalid PostgreSQL byte length"))?;
        Ok(self.take(length)?.to_vec())
    }

    fn string(&mut self) -> Result<String, StorageError> {
        String::from_utf8(self.bytes()?).map_err(|_| corrupt("invalid PostgreSQL UTF-8"))
    }
    ...
    fn digest(&mut self) -> Result<Digest32, StorageError> {
        Ok(Digest32::new(
            self.take(32)?
                .try_into()
                .map_err(|_| corrupt("invalid PostgreSQL digest"))?,
        ))
    }
    ...
    fn properties(&mut self, depth: usize) -> Result<Properties, StorageError> {
        if depth > MAX_NESTING {
            return Err(corrupt(
                "PostgreSQL property nesting exceeds provider bound",
            ));
        }
        let count = self.u32()?;
        ...
        for _ in 0..count {
            ...
            if values.insert(name, value).is_some() {
                return Err(corrupt("duplicate PostgreSQL property name"));
            }
        }
        Ok(values)
    }
    ...
    fn value(&mut self, depth: usize) -> Result<Value, StorageError> {
        if depth > MAX_NESTING {
            return Err(corrupt("PostgreSQL value nesting exceeds provider bound"));
        }
        match self.u8()? {
            0 => Ok(Value::Null),
            1 => match self.u8()? {
                ...
                _ => Err(corrupt("invalid PostgreSQL boolean")),
            },
            ...
            _ => Err(corrupt("unknown PostgreSQL value tag")),
        }
    }
    ...
    fn transaction(&mut self) -> Result<TransactionRecord, StorageError> {
        ...
        let state = match self.u8()? {
            ...
            _ => return Err(corrupt("unknown PostgreSQL transaction state")),
        };
    ...
    fn mutation(&mut self) -> Result<LogicalMutation, StorageError> {
        match self.u8()? {
            ...
            _ => Err(corrupt("unknown PostgreSQL mutation tag")),
        }
    }
}
...
fn kernel_error(error: dtg_kernel::KernelError) -> StorageError {
    StorageError::Internal(format!("invalid typed PostgreSQL value: {error}"))
}
