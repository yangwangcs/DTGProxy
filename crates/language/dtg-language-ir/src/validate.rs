...
use crate::{
    LogicalNodeId, LogicalNodeKind, LogicalPlan, LogicalProgram, LogicalStatement, TimeExpr,
};
...
pub fn validate_program(program: &LogicalProgram) -> Result<(), IrError> {
    ...
    match &program.statement {
        ...
        LogicalStatement::SubmitAnalytics(submission) => {
            validate_scope(&submission.temporal_scope, &parameters)
        }
        LogicalStatement::Write(_)
...
fn validate_node(
    ...
    match node {
        LogicalNodeKind::NodeScan(scan) => validate_scope(&scan.temporal_scope, parameters),
        LogicalNodeKind::RelationshipScan(scan) => validate_scope(&scan.temporal_scope, parameters),
        LogicalNodeKind::Expand(expand) => require(expand.input),
        LogicalNodeKind::Filter { input, .. }
        | LogicalNodeKind::Project { input, .. }
        | LogicalNodeKind::Aggregate(crate::Aggregate { input, .. })
        | LogicalNodeKind::Sort(crate::Sort { input, .. })
        | LogicalNodeKind::Limit(crate::Limit { input, .. })
        | LogicalNodeKind::Unwind(crate::Unwind { input, .. }) => require(*input),
