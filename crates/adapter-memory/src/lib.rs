...
use storage_api::{
    ...
    CanonicalBatchScanRequest, CanonicalScanPage, CanonicalScanRequest, ChangeScanPage,
    ChangeScanRequest, CommittedMutationBatch, Durability, KeySpan, KeyValue, LogicalKey,
    MutationOperation, PushdownGuarantee, QueryPrimitiveCapabilities, ReadSnapshot,
    ...
    fn scan_canonical_batch<'a>(
        ...
        Box::pin(async move {
            ...
            for scan in request.scans() {
                let (entries, next_start) = bounded_page(&self.data, scan.span(), scan.bounds())?;
